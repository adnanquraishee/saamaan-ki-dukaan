...
export default function Dashboard() {
  ...
  const [section, setSection] = useState<"overview" | "orders" | "agents" | "performance">("overview");
  ...
  return (
    <div className="tower min-h-screen bg-tower-bg text-tower-text">
      {!hydrated ? (
        ...
        <>
          <TowerHeader />
          <div className="border-b border-tower-line bg-[#0a1118] px-3 py-3"><div className="mx-auto flex max-w-[1500px] flex-wrap gap-2">{([['overview','Overview','Run the operation'],['orders','Orders','Trace every order'],['agents','Agents','See who is working'],['performance','Performance','Review outcomes']] as const).map(([id,label,help]) => <button key={id} onClick={() => setSection(id)} className={`rounded-sm border px-4 py-2 text-left transition ${section === id ? "border-tower-cyan bg-tower-cyan/10" : "border-tower-line hover:border-tower-dim"}`}><div className={`font-mono text-xs font-semibold ${section === id ? "text-tower-cyan" : "text-white"}`}>{label}</div><div className="mt-0.5 text-[10px] text-tower-dim">{help}</div></button>)}</div></div>
          <main className="mx-auto flex max-w-[1500px] flex-col gap-4 p-3 sm:p-5"><div key={section} className="dashboard-tab-panel flex flex-col gap-4">
            ...
            {section === "agents" && <><div><h2 className="text-xl font-semibold text-white">Agent operations</h2><p className="mt-1 text-sm text-tower-dim">Each agent reports its current status and last action.</p></div><div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]"><AgentRoster /><EscalationQueue /></div></>}
